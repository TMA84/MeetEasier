...
*/
function _isPlaceholderApiTokenValue(value) {
...
function decryptOAuthSecret(payload, tokenOverride) {
  ...
  console.error('Failed to decrypt OAuth client value: no valid API key found.');
  return null;
}
...
*/
/**
 * Resolves OAuth config from environment variables.
 * @param {Object} fallback
 * @returns {Object|null} Config if env is configured, null otherwise
 */
function _resolveOAuthFromEnv(fallback) {
  ...
  if (envClientId === undefined && envAuthority === undefined && envClientSecret === undefined) {
    return null;
  }
  return {
...
/**
 * Resolves OAuth config from file-based configuration.
 * @param {Object} fallback
 * @returns {Object}
 */
function _resolveOAuthFromFile(fallback) {
  const rawConfig = readRawOAuthConfig();
  if (!rawConfig) {
    return { ...fallback, lastUpdated: null };
  }
...
function getOAuthRuntimeConfig() {
  ...
  return _resolveOAuthFromEnv(fallback) || _resolveOAuthFromFile(fallback);
}
...
/**
 * Checks whether a certificate is available for MSAL authentication.
 * @returns {boolean}
 */
function _hasMsalCertificate() {
  try {
...
/**
 * Checks whether OAuth is fully configured (identity + secret or certificate).
 * @returns {boolean}
 */
function _isOAuthFullyConfigured() {
  ...
  if (!hasValidIdentity) {
    ...
    return !!oauthConfig.clientSecret || _hasMsalCertificate();
  }

  const hasSecret = clientSecret && clientSecret !== 'OAUTH_CLIENT_SECRET_NOT_SET';
  return hasSecret || _hasMsalCertificate();
}
  ...
  const oauthConfigured = _isOAuthFullyConfigured();
...
/**
* Merges a new value with an existing value, using a default fallback.
* Returns newVal if defined, else existingVal if defined, else defaultVal.
* @param {*} newVal - New value from input
* @param {*} existingVal - Existing value from config
* @param {*} defaultVal - Default fallback
* @returns {*}
*/
function mergeConfigValue(newVal, existingVal, defaultVal) {
...
function saveSidebarConfig(sidebarConfig) {
  ...
  const configData = {
    showWiFi: mergeConfigValue(sidebarConfig.showWiFi, existingConfig.showWiFi, true),
    showUpcomingMeetings: mergeConfigValue(sidebarConfig.showUpcomingMeetings, existingConfig.showUpcomingMeetings, false),
    showMeetingTitles: mergeConfigValue(sidebarConfig.showMeetingTitles, existingConfig.showMeetingTitles, false),
...
async function updateWifiApiToken(nextToken) {
  ...
  if (io) {
    ...
    console.log('WiFi API auth updated.');
  }
...
/** @function getMqttConfig - Reads the MQTT configuration from file or returns defaults */
module.exports.getMqttConfig = getMqttConfig;
/** @function updateMqttConfig - Updates the MQTT configuration and optionally restarts the client */
