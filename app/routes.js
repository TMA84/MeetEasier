...
/**
 * Validates booking input fields and returns an error response object or null if valid.
 * @param {Object} params - { roomEmail, subject, startTime, endTime, description, body }
 * @returns {Object|null} Error response { status, body } or null
 */
/**
 * Validates basic booking required fields.
 */
function _validateBookingRequiredFields(roomEmail, subject, startTime, endTime) {
  if (!subject || !startTime || !endTime) {
    return { status: 400, body: { error: 'Missing required fields', message: 'Subject, start time, and end time are required' } };
  }
  if (!isValidEmailAddress(roomEmail)) {
    return { status: 400, body: { error: 'Invalid room email', message: 'The room email format is invalid.' } };
  }
  return null;
}

/**
 * Validates booking field lengths and formats.
 */
function _validateBookingFieldLengths(subject, startTime, endTime, description) {
  const normalizedSubject = String(subject || '').trim();
  if (!normalizedSubject || normalizedSubject.length > 160) {
    return { status: 400, body: { error: 'Invalid subject', message: 'Subject is required and must not exceed 160 characters.' } };
  }
  const normalizedDescription = description === undefined || description === null ? '' : String(description);
  if (normalizedDescription.length > 2000) {
    return { status: 400, body: { error: 'Invalid description', message: 'Description must not exceed 2000 characters.' } };
  }
  ...
  if (!normalizedStartTime || !normalizedEndTime || normalizedStartTime.length > 64 || normalizedEndTime.length > 64) {
    return { status: 400, body: { error: 'Invalid date range', message: 'Start and end time values are invalid.' } };
  }
  return null;
}

function validateBookingInput(params) {
  const { roomEmail, subject, startTime, endTime, description, body } = params;
  const requiredError = _validateBookingRequiredFields(roomEmail, subject, startTime, endTime);
  if (requiredError) return requiredError;
  const lengthError = _validateBookingFieldLengths(subject, startTime, endTime, description);
  if (lengthError) return lengthError;
  const disallowedFields = ['attendees', 'requiredAttendees', 'optionalAttendees', 'resources', 'locations'];
  for (const field of disallowedFields) {
    if (body[field] !== undefined) {
      return { status: 400, body: { error: 'Invalid fields', message: 'Cannot add attendees or additional resources to room bookings' } };
    }
  }
  return null;
}
...
/**
 * Creates a translation object for extend-meeting error messages.
 * @param {boolean} isGerman
 * @returns {Object}
 */
function getExtendMeetingTranslations(isGerman) {
  return {
    missingFields: isGerman ? 'Fehlende erforderliche Felder' : 'Missing required fields',
    missingFieldsDetails: isGerman ? 'Raum-E-Mail, Termin-ID und Minuten sind erforderlich' : 'Room email, appointment ID, and minutes are required',
    extendDisabled: isGerman ? 'Meeting-Verlängerung deaktiviert' : 'Extend meeting disabled',
    extendDisabledDetails: isGerman ? 'Die Meeting-Verlängerung ist in der Admin-Konfiguration deaktiviert' : 'Extend meeting is disabled in the admin configuration',
    invalidMinutes: isGerman ? 'Ungültiger Minutenwert' : 'Invalid minutes value',
    invalidMinutesDetails: isGerman ? 'Minuten müssen zwischen 5 und 240 liegen (Schritte von 5)' : 'Minutes must be between 5 and 240 (steps of 5)',
    conflictError: isGerman ? 'Meeting kann nicht verlängert werden - ein weiterer Termin ist zu bald geplant. Bitte überprüfen Sie den Raumkalender.' : 'Cannot extend meeting - another meeting is scheduled too soon. Please check the room calendar.',
    endOfDayError: isGerman ? 'Meeting kann nicht über das Tagesende hinaus verlängert werden' : 'Cannot extend meeting beyond end of day',
...
/**
 * Creates a translation object for end-meeting error messages.
 * @param {boolean} isGerman
 * @returns {Object}
 */
function getEndMeetingTranslations(isGerman) {
  return {
    missingFields: isGerman ? 'Fehlende erforderliche Felder' : 'Missing required fields',
    missingFieldsDetails: isGerman ? 'Raum-E-Mail und Termin-ID sind erforderlich' : 'Room email and appointment ID are required',
    endDisabled: isGerman ? 'Meeting-Beenden deaktiviert' : 'End meeting disabled',
    endDisabledDetails: isGerman ? 'Meeting-Verwaltung ist in der Admin-Konfiguration deaktiviert' : 'Meeting management is disabled in the admin configuration',
 ...
 * @returns {boolean} Whether the language is German
 */
function isRequestGerman(req) {
...
/**
 * Validates OAuth sentinel values in a request body.
 * @param {Object} body - Request body with clientId, authority, tenantId, clientSecret
 * @returns {Object|null} Error response { status, body } or null
 */
function validateOAuthSentinels(body) {
  const { clientId, authority, tenantId, clientSecret } = body || {};
  const invalidSentinelValues = new Set(['OAUTH_CLIENT_ID_NOT_SET', 'OAUTH_AUTHORITY_NOT_SET', 'OAUTH_CLIENT_SECRET_NOT_SET']);
  const checks = [
    { val: clientId, label: 'client ID', msg: 'Please provide a real OAuth Client ID (placeholder values are not allowed).' },
    { val: authority, label: 'authority', msg: 'Please provide a real OAuth Tenant ID or Authority URL (placeholder values are not allowed).' },
    { val: tenantId, label: 'tenant ID', msg: 'Please provide a real OAuth Tenant ID (placeholder values are not allowed).' },
    { val: clientSecret, label: 'client secret', msg: 'Please provide a real OAuth Client Secret (placeholder values are not allowed).' }
  ];
  for (const check of checks) {
    if (typeof check.val === 'string' && invalidSentinelValues.has(check.val.trim())) {
      return { status: 400, body: { error: `Invalid OAuth ${check.label}`, message: check.msg } };
    }
  }
  return null;
}
...
/**
 * Validates OAuth field formats (GUID for clientId/tenantId, authority URL).
 * @param {Object} body
 * @returns {Object|null} Error response or null
 */
function validateOAuthFormats(body) {
  const { clientId, authority, tenantId } = body || {};
  if (clientId !== undefined) {
    const normalized = String(clientId || '').trim();
    if (!isValidGuid(normalized)) {
      return { status: 400, body: { error: 'Invalid OAuth client ID', message: 'Client ID must be a valid GUID.' } };
    }
  }
  if (tenantId !== undefined) {
    const normalized = String(tenantId || '').trim();
    if (!isValidGuid(normalized)) {
      return { status: 400, body: { error: 'Invalid OAuth tenant ID', message: 'Tenant ID must be a valid GUID.' } };
    }
  }
  if (authority !== undefined && tenantId === undefined) {
    const normalized = String(authority || '').trim();
    if (!isValidMicrosoftAuthority(normalized)) {
      return { status: 400, body: { error: 'Invalid OAuth authority', message: 'Authority must be a valid login.microsoftonline.com URL with tenant segment.' } };
    }
  }
  return null;
}
...
/**
 * Validates API token update payload.
 * @param {Object} params - { newToken, newWifiToken, isAdminLocked, isWifiLocked }
 * @returns {Object|null} Error response or null
 */
function validateApiTokenUpdate(params) {
  const { newToken, newWifiToken, isAdminLocked, isWifiLocked } = params;
  const hasNewAdminToken = newToken !== undefined;
  const hasNewWiFiToken = newWifiToken !== undefined;
  if (!hasNewAdminToken && !hasNewWiFiToken) {
    return { status: 400, body: { error: 'Invalid token update payload', message: 'Please provide newToken and/or newWifiToken.' } };
  }
  if (hasNewAdminToken) {
    const err = _validateSingleToken(newToken, isAdminLocked, 'API token', 'API_TOKEN');
    if (err) return err;
  }
  if (hasNewWiFiToken) {
    const err = _validateSingleToken(newWifiToken, isWifiLocked, 'WiFi API token', 'WIFI_API_TOKEN');
    if (err) return err;
  }
  return null;
}

function _validateSingleToken(token, isLocked, label, envVar) {
  if (isLocked) {
    return { status: 403, body: { error: `${label} is locked by environment variable`, message: `Remove ${envVar} from environment to edit via admin panel.` } };
  }
  const normalized = String(token || '').trim();
  if (!normalized || normalized.length < 8) {
    return { status: 400, body: { error: `Invalid ${label}`, message: `Please provide a ${label} with at least 8 characters.` } };
  }
  return null;
}

/**
 * Handles demo mode for booking-related endpoints.
 * @returns {Object|null} Response object if demo mode handled, null otherwise
 */
function handleDemoBooking(systemConfig, roomEmail, action, params) {
  if (!systemConfig.demoMode || !demoData.isDemoEmail(roomEmail)) return null;
  let result;
  if (action === 'book') {
    result = demoData.bookDemoRoom(roomEmail, params.subject, params.startTime, params.endTime);
    if (result.error) return { status: result.status, body: { error: 'Room not available', message: result.message } };
    return { status: 200, body: { success: true, id: result.id, subject: result.subject }, emitRooms: true };
  }
  if (action === 'extend') {
    result = demoData.extendDemoMeeting(roomEmail, params.appointmentId, Number(params.minutes));
    if (result.error) return { status: result.status, body: { success: false, error: result.message } };
    return { status: 200, body: { success: true, newEnd: result.newEnd }, emitRooms: true };
  }
  if (action === 'end') {
    result = demoData.endDemoMeetingEarly(roomEmail, params.appointmentId);
    if (result.error) return { status: result.status, body: { success: false, error: result.message } };
    return { status: 200, body: { success: true, endedAt: result.endedAt }, emitRooms: true };
  }
  return null;
}
  ...
  try {
    ...
    if (response && response.accessToken) {
      ...
      if (parts.length === 3) {
        ...
        // Acquired but Calendars.ReadWrite not in roles — don't cache, might be a timing issue
        console.log('✗ Calendars.ReadWrite not found in acquired roles:', roles.join(', '));
        return false;
      }
      // Could not decode token — assume permission is available if token was obtained
      hasCalendarWritePermission = true;
      console.log('✓ Auth acquired (could not decode roles) - Booking feature enabled');
      return true;
    }
    
    console.log('✗ No auth response received - Booking feature disabled');
    return false;
  } catch (error) {
...
module.exports = function(app) {
  ...
  */
  ...
  app.post('/api/rooms/:roomEmail/book', checkDisplayOrigin, function(req, res, next) {
    ...
    const systemConfig = configManager.getSystemConfig();
    const demoResult = handleDemoBooking(systemConfig, roomEmail, 'book', { subject, startTime, endTime });
    if (demoResult) {
      if (demoResult.emitRooms) { const io = configManager.getSocketIO(); if (io) io.of('/').emit('updatedRooms', demoData.getDemoRoomsSnapshot()); }
      return res.status(demoResult.status).json(demoResult.body);
    }
    ...
    // Input validation
    const validationError = validateBookingInput({ roomEmail, subject, startTime, endTime, description, body: req.body });
    if (validationError) {
  ...
  app.post('/api/extend-meeting', checkDisplayOrigin, function(req, res, next) {
    ...
    const systemConfig = configManager.getSystemConfig();
    const demoResult = handleDemoBooking(systemConfig, roomEmail, 'extend', { appointmentId, minutes });
    if (demoResult) {
      if (demoResult.emitRooms) { const io = configManager.getSocketIO(); if (io) io.of('/').emit('updatedRooms', demoData.getDemoRoomsSnapshot()); }
      return res.status(demoResult.status).json(demoResult.body);
    }
    ...
    const isGerman = isRequestGerman(req);
    const t = getExtendMeetingTranslations(isGerman);
    // Input validation
  ...
  app.post('/api/end-meeting', checkDisplayOrigin, function(req, res, next) {
    ...
    const systemConfig = configManager.getSystemConfig();
    const demoResult = handleDemoBooking(systemConfig, roomEmail, 'end', { appointmentId });
    if (demoResult) {
      if (demoResult.emitRooms) { const io = configManager.getSocketIO(); if (io) io.of('/').emit('updatedRooms', demoData.getDemoRoomsSnapshot()); }
      return res.status(demoResult.status).json(demoResult.body);
    }
    ...
    const t = getEndMeetingTranslations(isGerman);
  ...
  */
  ...
  app.post('/api/admin/bootstrap-token', function(req, res) {
    ...
    configManager.updateApiToken(providedToken)
      ...
      .catch((err) => {
        console.error('Error creating initial admin API auth:', err);
  ...
  app.get('/api/api-token-config', checkApiToken, function(req, res) {
    try {
      ...
      console.error('Error retrieving API auth config:', err);
  ...
  app.post('/api/api-token-config', checkApiToken, async function(req, res) {
    try {
      ...
      const tokenError = validateApiTokenUpdate({
        newToken,
        newWifiToken,
        isAdminLocked: hasNewAdminToken && configManager.isApiTokenEnvLocked(),
        isWifiLocked: hasNewWiFiToken && configManager.isWifiApiTokenEnvLocked && configManager.isWifiApiTokenEnvLocked()
      });
      if (tokenError) {
      ...
      console.error('Error updating API auth:', err);
  ...
  app.post('/api/oauth-config', checkApiToken, async function(req, res) {
    try {
      ...
      const sentinelError = validateOAuthSentinels(req.body);
      if (sentinelError) {
        return res.status(sentinelError.status).json(sentinelError.body);
      }
      ...
      const formatError = validateOAuthFormats(req.body);
      if (formatError) {
        return res.status(formatError.status).json(formatError.body);
      }
